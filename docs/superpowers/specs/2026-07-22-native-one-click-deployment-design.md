...
M0A.5 完成后不再存在认证模式开关。loopback HTTP 可以使用非 Secure Cookie；任何非 loopback 公共入口必须使用 HTTPS 和 Secure Cookie，否则部署拒绝启动。test 邮件模式只允许 loopback 公共地址。
...
- Web `3000`，API `8787`。
- DataLink 关闭。
- password-only 认证、开放注册和 test 邮件模式。
...
远程服务器的临时验证默认通过 SSH 端口转发访问 `http://127.0.0.1:${WEB_PORT}`。需要局域网或公网访问时，部署者必须提供 HTTPS 反向代理和正确的 `AUTH_PUBLIC_BASE_URL`；原生部署不得直接暴露非 loopback HTTP。
...
- test 邮件模式产生的验证令牌属于敏感运行信息，只允许在 loopback 部署中返回或写入权限受限的运行日志，不进入部署摘要或诊断分享内容。
